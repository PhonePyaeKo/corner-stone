            ...
            <div class="px-4 py-6 sm:p-8">
                <div class="space-y-12">
                    <div class="border-b border-gray-900/10 pb-3">
                        <div class="text-base/7 font-semibold text-gray-900">{{ __('global.edit') }}
                            {{ __('labels.user.title_singular') }}</div>
                        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <div class="mt-2">
                                    <input type="text" name="name" id="name"
                                        value="{{ old('name', $user->name) }}" autocomplete="given-name" required
                            ...
                            <div class="sm:col-span-full">
                                <fieldset>
                                    ...
                                    <div class="sm:py-3">
                                        <div class="text-sm/6 font-semibold text-gray-900" aria-hidden="true">By Roles</div>
                ...
                <div class="mt-6 flex items-center justify-end gap-x-6">
                    <a href="{{ route('admin.users.index') }}"
                        class="text-sm rounded-md font-semibold custom-color hover:outline px-3 py-2 cursor-pointer">
                        {{ __('global.cancel') }}
                    </a>
                    <button type="submit"
...
@section('scripts')
    <script>
        ...
        Dropzone.options.profileImageDropzone = {
            ...
            init: function() {
                @if (isset($user) &&
                        $user->getMedia('profile_image')->first() &&
                        $user->getMedia('profile_image')->first() !== '/user-avatar.png')
                    var fileName = {!! json_encode(\App\Models\User::getImageName($user->profile_image)) !!};
                    var mockFile = {
                        name: fileName,
                        size: 2,
                        accepted: true
                    ...
                    var publicUrl = '';
                    var imgPath = {!! json_encode($user->getMedia('profile_image')->first()->getFullUrl()) !!};

                    if (imgPath.startsWith('/profile_images/')) {
                        publicUrl = imgPath;
                    } else {
                        publicUrl = imgPath;
                    }
